...
public class NTTileResult {
    ...
    public NTTileResult(RectD rectD, NormalPave normalPave) {
        this.box = rectD;
    ...
    private void saveGap(Area3D area3D) {
        if (area3D == null)
            return;
        int color = normalPave.getGapsColor();
        if (color == 0) { // 黑色砖缝
            color = 0xFF000000;
        ...
        this.gap = this.normalPave.getBrickGap();
    ...
    private void saveArea3DBitmap(Area3D area3D) {
        Bitmap codeBitmap = normalPave.getOriginBitmap();
